  ...
  describe 'scopes' do
    ...
    describe '.by_breed' do
      it 'returns cats of specific breed' do
        expect(Cat.by_breed('Persian')).to include(persian_cat)
        expect(Cat.by_breed('Persian')).not_to include(siamese_cat)
      end
    end

    describe '.by_age_range' do
      it 'returns cats within age range' do
        cats_in_range = Cat.by_age_range(2, 4)
    ...
    describe '.by_color' do
      it 'returns cats of specific color' do
        expect(Cat.by_color('White')).to include(persian_cat)
        expect(Cat.by_color('White')).not_to include(siamese_cat)
      end
    end
  end
end
